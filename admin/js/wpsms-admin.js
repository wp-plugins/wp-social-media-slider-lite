/* CodeMirror - Minified & Bundled
   Generated on 2/16/2015 with http://codemirror.net/doc/compress.html
   Version: HEAD
...
(function( $ ) {
	...
	$( function() {
		...
		wpsmsConnection = {
			...
			/**
			 * Perform an ajax check to see if a network is connected
			 *
			 * @since   1.1.2
			 * @param   {string}  action     The ajax action that will be posted to
			 * @param   {string}  container  the location that will recieve the output
			 * @return  {void}
			 */
			checkStatus: function ( action, container ) {
				$.ajax({
					url: ajaxurl,
					type: 'POST',
					data: { action: action },
				...
				.done( function( response ) {
					...
					if ( response.hasOwnProperty( 'html' ) ) {
						...
						$( ".wpsms-info" ).magnificPopup({
						  type: 'inline',
						  midClick: true
						});

						if( response.status == 'connected' ) {
							$( container ).addClass( 'wpsms-connected' );
						} else {
							$( container ).addClass( 'wpsms-not-connected' );
						}
					}
					else {
						$( container ).html( "Connection check failed." ).addClass( 'wpsms-not-connected' );
					}
...
(function( $ ) {
	'use strict';
	 ...
	 $( function() {
		...
		if ( $( "#wpsms-custom-js-init" ).length ) {
		 	CodeMirror.fromTextArea(document.getElementById("wpsms-custom-js-init"), {
				lineNumbers: true,
				mode: "javascript"
			});
		}
		...
		$( ".wpsms-info" ).magnificPopup({
	 	...
	 	if ( is_ie_8 ) {
		 	// Pre-check boxes where neccessary
		 	$(".onoffswitch-label").each( function() {
				var checkbox = $( '#' + $( this ).attr( 'for' ));

			      if( checkbox.is(":checked") ) {
			        $( this ).find( '.onoffswitch-inner' ).addClass("checkedinner" );
			        $( this ).find( '.onoffswitch-switch' ).addClass("checkedswitch" );
			      }
			});
			// Support checkboxes in IE8
			$(".onoffswitch-label").on('click', function() {
				...
				//alert( checkbox.attr('id') );
			      ...
			      if( checkbox.is(":checked") ) {
			        ...
			        $( this ).find( '.onoffswitch-inner' ).removeClass("checkedinner" );
			        $( this ).find( '.onoffswitch-switch' ).removeClass("checkedswitch" );
			      } else {
